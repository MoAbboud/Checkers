...
import sprint3_prod.Board.GameState;
import sprint3_prod.LoginController;
...
public class CheckersGUI {
	...
	@FXML
	private Label lblPlayer1Win;


	@FXML
	private Label lblPlayer1Loss;
	...
	@FXML
	private Label lblPlayer2Win;
	
	@FXML
	private Label lblPlayer2Loss;
	...
	public void playerNames(String name1, int win1, int loss1, String name2, int win2, int loss2, int pid1, int pid2) {	
		lblPlayer1.setText(name1);	
		lblPlayer2.setText(name2);	
		lblPlayer1Win.setText(Integer.toString(win1));
		lblPlayer1Loss.setText(Integer.toString(loss1));
		playerOne = pid1;
		playerTwo = pid2;
		lblPlayer2Win.setText(Integer.toString(win2));
		lblPlayer2Loss.setText(Integer.toString(loss2));
					...
					piece.setOnMouseClicked(new EventHandler<MouseEvent>() {
						...
						public void handle(MouseEvent event) {
							...
							if (piece.getBoundsInParent().contains(event.getSceneX(), event.getSceneY())) {
								...
								final int fromRow = CheckRowIndex(piece);
								final int fromCol = CheckColIndex(piece);
													...
													if (row1 == toRow && col1 == toCol || row2 == toRow && col2 == toCol
														...
														if (jumpMovesFlag) {
															...
															if (board.getGameState() == GameState.PLAYING) {
																...
																if (board.getTurn().equals("Black")) lblState.setText("Black turns!"); 
																else lblState.setText("White turns!");
	...
	//writing black win case adding win with 1, adding loss with 1 to white
	private void recordBlackWin() {
		 connection = JavaConnection2SQL.ConnectDB();
    	 String sql1 = "UPDATE PLAYER SET P_WIN = ? WHERE P_ID = ?";
    	 String sql2 = "UPDATE PLAYER SET P_LOSS = ? WHERE P_ID = ?";
    	 
    	 try {
    		 pst = connection.prepareStatement(sql1);
    		 pst.setString(1, Integer.toString(Integer.parseInt(lblPlayer1Win.getText()) + 1));
     		 pst.setString(2, Integer.toString(playerOne));
    		 pst.execute();
    		 pst = connection.prepareStatement(sql2);
    		 pst.setString(1, Integer.toString(Integer.parseInt(lblPlayer2Loss.getText()) + 1));
     		 pst.setString(2, Integer.toString(playerTwo));
    		 pst.execute();
	...
	//writing white win case adding win with 1, adding loss with 1 to black
	private void recordWhiteWin() {
		 connection = JavaConnection2SQL.ConnectDB();
    	 String sql1 = "UPDATE PLAYER SET P_LOSS = ? WHERE P_ID = ?";
    	 String sql2 = "UPDATE PLAYER SET P_WIN = ? WHERE P_ID = ?";
    	 
    	 try {
    		 pst = connection.prepareStatement(sql1);
    		 pst.setString(1, Integer.toString(Integer.parseInt(lblPlayer1Loss.getText()) + 1));
     		 pst.setString(2, Integer.toString(playerOne));
    		 pst.execute();
    		 pst = connection.prepareStatement(sql2);
    		 pst.setString(1, Integer.toString(Integer.parseInt(lblPlayer2Win.getText()) + 1));
     		 pst.setString(2, Integer.toString(playerTwo));
    		 pst.execute();
